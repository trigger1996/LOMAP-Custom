...
def main():
    ...
    with Timer('TASE 2021 Complex'):
        ...
        r7  = Ts.load('./robot_7.yaml')
        r8  = Ts.load('./robot_8.yaml')
        r9  = Ts.load('./robot_9.yaml')
        r10 = Ts.load('./robot_10.yaml')
        r11 = Ts.load('./robot_11.yaml')

        # CASE 6 -- 2 controllable succeeded
        '''
        ts_tuple = (r1, r2, r5, r6, r7, r8, r9, r10)
        is_modifible = [True, True, False, False, False, False, False, False]
        formula = ('[]<>gather && [](gather->((r1gather1 || r1gather2) && r2gather3)) '
                   '&& [](r1gather -> X(!r1gather U r1upload1)) '
                   '&& [](r2gather -> X(!r2gather U r2upload2))')
        opt_prop = set(['r1gather','r2gather'])
        '''
        # CASE 6 -- 2 controllable constrainted
        r1_1 = Ts.load('./robot_1-1.yaml')
        r2_1 = Ts.load('./robot_2-1.yaml')
        r3_1 = Ts.load('./robot_3-1.yaml')              # Ts.load('./robot_3-2.yaml')
        r4_1 = Ts.load('./robot_4-1.yaml')

        ts_tuple = (r1_1, r2_1, r3_1, r5, r6, r7, r8, r9, r10, r11)
        is_modifible = [True, True, True, False, False, False, False, False, False, False]
        formula = ('[]<>gather && [](gather->((r1gather1 || r1gather2) && r2gather3 && r3gather)) '
                   '&& [](r1gather -> X(!r1gather U r1upload1)) '
                   '&& [](r2gather -> X(!r2gather U r2upload2)) '
                   '&& [](r3gather -> X(!r3gather U r3upload3))')
        opt_prop = set(['r1gather','r2gather','r3gather'])

        # For Testing
        '''
        ts_tuple = (r1_1, r4_1)
        is_modifible = [True, True, True, False, False, False, False, False]
        formula = ('[]<>gather && [](gather->(r1gather && r4gather)) '
                   '&& [](r1gather -> X(!r1gather U r1upload)) '
                   '&& [](r4gather -> X(!r4gather U r4upload))')
        opt_prop = set(['r1gather','r4gather'])
        '''
    ...
    # animations
    view_complex_3.visualize_animation_w_team_run(ts_tuple, team_suffix_cycle)
