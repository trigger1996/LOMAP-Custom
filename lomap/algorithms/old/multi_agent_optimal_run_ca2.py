...
class Graph2(ig.Graph):
    ...
    def shortest_paths_nx_no_degenerated_path(self, source=None, target=None, weights=None, mode='out'):
        ...
        # 2.2 find non-degenerate path for vertices
        for source_t in source:
            ...
            seen = dict()
            for edge_t in self.es.select(_source=self.vs.find(nx_name=source_t)):
                vw_dist = edge_t[weights]
                seen[edge_t.target] = vw_dist
                heapq.heappush(fringe, (vw_dist, edge_t.target))

            while fringe:
                (d_t, v) = heapq.heappop(fringe)

                if v in dist:
                    continue  # Already searched this node.

                dist[self.vs[v]['nx_name']] = d_t  # Update distance to this node

                for edge_t in self.es.select(_source=v):
                    vw_dist = dist[self.vs[v]['nx_name']] + edge_t[weights]
                    if self.vs[edge_t.target]['nx_name'] in dist:
                        if vw_dist < dist[self.vs[edge_t.target]['nx_name']]:
                            raise ValueError('Contradictory paths found:', 'negative weights?')
                    elif edge_t.target not in seen or vw_dist < seen[edge_t.target]:
                        seen[edge_t.target] = vw_dist
                        heapq.heappush(fringe, (vw_dist, edge_t.target))
...
def min_bottleneck_cycle(g, s, f, pp_workers=4):
    ...
    f_to_s_length = product_automata_2.shortest_paths_nx(list(f), list(s), weights='weight')
    '''
    ...
    (cost_star_ig, len_star_ig, cycle_star_ig) = find_best_cycle_ig(f_index_list, s_index_list, f_to_s_length, s_to_f_length, d_bot_ig)
